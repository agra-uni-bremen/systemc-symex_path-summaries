...
public:
  ...
  bool newThread=false; // was new thread created? necessary for correct setting of mainpc/stack AFTER ptc-exec hack
  bool registerNew=false; // should new thread be registered on cond-wait?
  ...
  /// @brief each thread's dynamic wait events
  std::map<uint64_t, std::vector<uint64_t>> threadEventsD;
  /// @brief each thread's static wait events
  std::map<uint64_t, std::vector<uint64_t>> threadEventsS;
  /// @brief map channels (signal/port) to their change event
  std::map<uint64_t, uint64_t> channelEvent;
  /// @brief map channels (signal/port) to their bool events
  std::map<uint64_t, std::pair<uint64_t,uint64_t>> channelBoolEvents;
  /// @brief map ports to their signal
  std::map<uint64_t, std::vector<uint64_t>> portSignal;
  ...
  bool symsim=false;
  std::vector<bool> simt = {};
  std::vector<llvm::BranchInst*> symBranches = {};
  std::vector<ref<Expr>> symCond = {};
  // constraintset at the time the branch point was reached => set accordingly
  std::vector<ConstraintSet> branchBackup = {};
...
public:
  ...
  bool areThreadsWaiting(uint64_t channel);
