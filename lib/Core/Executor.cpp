...
cl::OptionCategory SymSimCat("SymSim-related options.");
...
cl::opt<bool> ThreadDebug(
    "thread-debug",
    cl::desc(
        "Debug output for PThread SystemC Threading"
        ),
    cl::init(false),
    cl::cat(SymSimCat)
);
...
cl::opt<bool> SymSim(
    "symsim",
    cl::desc(
        "Use symbolic simulation (no forking, build complex expression) for certain things"
        ),
    cl::init(false),
    cl::cat(SymSimCat)
);

cl::opt<bool> SymSimSolver(
    "symsim-solver",
    cl::desc(
        "Use solver during symsim to determine valid paths"
        ),
    cl::init(false),
    cl::cat(SymSimCat)
);

cl::opt<bool> SymSimDebug(
    "symsim-debug",
    cl::desc(
        "Show debug information during SymSim."
        ),
    cl::init(false),
    cl::cat(SymSimCat)
);

cl::opt<bool> TurnSymbolicLocal(
    "symsim-local",
    cl::desc(
        "Make concrete values symbolic during symsim to avoid paths"
        ),
    cl::init(false),
    cl::cat(SymSimCat)
);

cl::opt<bool> TurnSymbolicGlobal(
    "symsim-global",
    cl::desc(
        "Make concrete values symbolic during symsim to avoid paths"
        ),
...
cl::opt<int> SymSimLoopCount(
    "symsim-loopcount",
    cl::desc(
        "How many loop iterations to explore with SymSim? (default=0)"),
    cl::init(0),
    cl::cat(SymSimCat));

cl::list<std::string> SymSimTrue(
    "symsim-true",
    cl::desc("If function has symbolic paths, always choose true"),
    cl::value_desc("mangled name as in LLVM-IR"),
    cl::cat(SymSimCat));

cl::list<std::string> SymSimFalse(
    "symsim-false",
    cl::desc("If function has symbolic paths, always choose false"),
    cl::value_desc("mangled name as in LLVM-IR"),
    cl::cat(SymSimCat));

cl::list<std::string> SymSimIgnore(
    "symsim-ignore",
    cl::desc("If function has symbolic paths, do not execute SymSim (fork)"),
    cl::value_desc("mangled name as in LLVM-IR"),
    cl::cat(SymSimCat));

cl::list<std::string> SymSimPrefixes(
    "symsim-prefix",
    cl::desc("Only do SymSim in files that match prefix(es)"),
    cl::value_desc("relative directory/file name"),
    cl::cat(SymSimCat));
...
Executor::StatePair Executor::fork(ExecutionState &current, ref<Expr> condition,
  ...
  if (res==Solver::True) {
    ...
    if(former) trueState = &current;
    else falseState = &current;
    ...
    if(former)
      ...
      executionTree->attach(current.executionTreeNode, trueState, falseState, reason);
...
bool Executor::addConstraint(ExecutionState &state, ref<Expr> condition) {
  ...
  bool success = state.addConstraint(condition);
  if (ivcEnabled)
...
void Executor::executeCall(ExecutionState &state, KInstruction *ki, Function *f,
  ...
  if (f && f->isDeclaration()) {
    ...
    if(state.newThread) {
      if(ThreadDebug) klee_warning("[threading] creating empty stack for new "
                     "thread, keeping all stackframe info for new frame");
...
uint64_t Executor::getConcreteAddress(klee::ExecutionState &state,
                                       ref<Expr> expr, bool zeroOffset) {
  expr = toUnique(state, expr);
  if (!isa<ConstantExpr>(expr)) {
    klee_warning("[threading] symbolic thread condition encountered in pthread-cond-...");
    return 0;
  }
  ref<ConstantExpr> constExpr = cast<ConstantExpr>(expr);
  ObjectPair op;
  if (!state.addressSpace.resolveOne(constExpr, op)) {
    klee_warning("[threading] invalid condition pointer passed to pthread-cond-...");
    return 0;
  }
  const MemoryObject *mo = op.first;

  auto relativeOffset = mo->getOffsetExpr(constExpr);
  size_t offset = cast<ConstantExpr>(relativeOffset)->getZExtValue();
  if(ThreadDebug) {
    if(offset==0) klee_warning("[threading] zero offset in memobj for condition");
    else {
      std::string n1 = "[threading] offset found: " + std::to_string(offset);
      klee_warning(n1.c_str());
    }
  }
  if(!zeroOffset && offset==0) return 0;
  uint64_t final_condition = constExpr->getZExtValue() + offset;

  if(ThreadDebug) {
    std::string n2 = "[threading] final condition address: " + std::to_string(final_condition);
    klee_warning(n2.c_str());
  }
...
void Executor::executeInstruction(ExecutionState &state, KInstruction *ki) {
  ...
  case Instruction::Ret: {
    ...
    if (!isVoidReturn) {
      ...
      if(state.stack.back().kf && state.stack.back().kf->function
          && (state.stack.back().kf->function->getName().contains("sc_core")
              &&state.stack.back().kf->function->getName().contains("sc_signal")
              &&state.stack.back().kf->function->getName().contains("value_changed_event"))) {
        // base _ZNK7sc_core17sc_signal_channel19value_changed_eventEv, but need signal / port instead
        ref<Expr> signalExpr = state.stack.back().locals[state.stack.back().kf->getArgRegister(0)].value;
        uint64_t signal = getConcreteAddress(state, signalExpr, true);
        uint64_t event = getConcreteAddress(state, result, true);
        state.channelEvent[signal] = event;
      } else if(state.stack.back().kf && state.stack.back().kf->function
                 && (state.stack.back().kf->function->getName().contains("sc_core")
                     &&state.stack.back().kf->function->getName().contains("sc_signal")
                     &&state.stack.back().kf->function->getName().contains("posedge_event"))) {
        // base _ZNK7sc_core17sc_signal_channel19value_changed_eventEv, but need signal / port instead
        ref<Expr> signalExpr = state.stack.back().locals[state.stack.back().kf->getArgRegister(0)].value;
        uint64_t signal = getConcreteAddress(state, signalExpr, true);
        uint64_t event = getConcreteAddress(state, result, true);
        state.channelBoolEvents[signal]; // add pair if not there yet
        state.channelBoolEvents[signal].first = event;
      } else if(state.stack.back().kf && state.stack.back().kf->function
                 && (state.stack.back().kf->function->getName().contains("sc_core")
                     &&state.stack.back().kf->function->getName().contains("sc_signal")
                     &&state.stack.back().kf->function->getName().contains("negedge_event"))) {
        // base _ZNK7sc_core17sc_signal_channel19value_changed_eventEv, but need signal / port instead
        ref<Expr> signalExpr = state.stack.back().locals[state.stack.back().kf->getArgRegister(0)].value;
        uint64_t signal = getConcreteAddress(state, signalExpr, true);
        uint64_t event = getConcreteAddress(state, result, true);
        state.channelBoolEvents[signal]; // add pair if not there yet
        state.channelBoolEvents[signal].second = event;
      }
    }
  ...
  case Instruction::Br: {
    ...
    if (bi->isUnconditional()) {
      ...
      cond = optimizer.optimizeExpr(cond, false);
      bool doFork = true;
      if(SymSim && !dyn_cast<ConstantExpr>(cond) && (bi->getFunction() && std::find(SymSimIgnore.begin(), SymSimIgnore.end(), bi->getFunction()->getName()) == SymSimIgnore.end())) {
          if(SymSimDebug) {
            std::string msg = "[symsim] symbolic condition now at state: ";

            llvm::raw_string_ostream outputLocal(msg);
            std::string infoPC = std::to_string(state.getID()) +
                                 ", [prevPC] id: " +
                                 std::to_string(state.prevPC->info->id) + ", line: " +
                                 std::to_string(state.prevPC->info->line) +
                                 ", assembly line: " +
                                 std::to_string(state.prevPC->info->assemblyLine) +
                                 ", source file name: " + state.prevPC->info->file;
            outputLocal << infoPC;
            klee_warning("%s", outputLocal.str().c_str());

            std::string qmsg = "[symsim] the condition in question: ";
            llvm::raw_string_ostream outputQuery(qmsg);
            ExprPPrinter::printQuery(outputQuery, state.constraints, cond);
            klee_warning("%s", outputQuery.str().c_str());
          }
          // check force conditions
          bool force=true;
          bool funcmatch=false;
          if(bi->getFunction() &&
              ((funcmatch=(bi->getFunction()->getName().contains("sc_core")
                             &&bi->getFunction()->getName().contains("sc_signal")
                             &&bi->getFunction()->getName().contains("writeE")))
              || (force = std::find(SymSimTrue.begin(), SymSimTrue.end(), bi->getFunction()->getName()) != SymSimTrue.end()))) {
            if(funcmatch) {
              ref<Expr> signalExpr =
                  state.stack.back()
                      .locals[state.stack.back().kf->getArgRegister(0)]
                      .value;
              uint64_t signal = getConcreteAddress(state, signalExpr, true);
              force = !state.areThreadsWaiting(signal);
            }
            if(force) {
              if (SymSimDebug) klee_warning("[symsim] we shall ignore this "
                             "condition, and choose the true path");
              doFork = false;
              transferToBasicBlock(bi->getSuccessor(0), bi->getParent(), state);
              if (statsTracker && state.stack.back().kf->trackCoverage)
                statsTracker->markBranchVisited(theStatisticManager->getIndex(),
                                                &state, nullptr);
            }
          } else if(bi->getFunction() &&
                     ((funcmatch=(bi->getFunction()->getName().contains("sc_core")
                                    &&bi->getFunction()->getName().contains("sc_signal")
                                    &&bi->getFunction()->getName().contains("updateE")))
                      || (force = std::find(SymSimFalse.begin(), SymSimFalse.end(), bi->getFunction()->getName()) != SymSimFalse.end()))) {
            if(funcmatch) {
              ref<Expr> signalExpr =
                  state.stack.back()
                      .locals[state.stack.back().kf->getArgRegister(0)]
            ...
            if(force) {
              if (SymSimDebug) klee_warning("[symsim] we shall ignore this "
                             "condition, and choose the false path");
              doFork = false;
          ...
          // check SymSim prefixes (user-defined)
          else if (checkSymSimPrefixes(ki->info->file)) {
            KFunction *kfun = kmodule->functionMap[bi->getFunction()];
            // check loop
            Loop *loop = kfun->loops.getLoopFor(bi->getParent());
            uint64_t loopcount = theStatisticManager->getIndexedValue(
                stats::symSimLoopCount, ki->info->id);
            if (!(loop && loop->isLoopExiting(bi->getParent())) ||
                loopcount < SymSimLoopCount) {
              if (loop && loop->isLoopExiting(bi->getParent()))
                theStatisticManager->setIndexedValue(stats::symSimLoopCount,
                                                     ki->info->id, loopcount+1);
              BasicBlock *t = bi->getSuccessor(0);
              BasicBlock *f = bi->getSuccessor(1);
              BasicBlock *end = kfun->postDom.findNearestCommonDominator(t, f);
              // check basic condition (common postdom)
              if (end) {
                if (SymSimDebug)
                  klee_warning("[symsim] fullexec, qualifies for symsim");

                // check Solver if necessary
                Solver::Validity res;
                if(SymSimSolver) {
                  if(SymSimDebug) klee_warning("[symsim] call solver for symsim condition");
                  cond = maxStaticPctChecks(state, cond);
                  ...
                  if (!success) {
                    ...
                    terminateStateOnSolverError(state, "Query timed out (symsim).");
                    return;
                  }
                  if(res == Solver::False && SymSimDebug) klee_warning("[symsim] not necessary, only false possible");
                  else if(res == Solver::True && SymSimDebug) klee_warning("[symsim] not necessary, only true possible");
                }
                // finally, do SymSim
                if(!SymSimSolver || res == Solver::Unknown) {
                  doFork = false;
                  state.symsim = true;
                  state.symBranches.push_back(bi);
                  state.symCond.push_back(cond);
                  state.branchBackup.push_back(state.constraints);
                  trySimBranch(state, true, t, f);
                }
              } else if (SymSimDebug) klee_warning("[symsim] not eligible for symsim");
            } else if (SymSimDebug) {
              if (!loop) klee_warning("[symsim] LOOP: did not find loop");
              klee_warning(
                  "[symsim] LOOP CONDITION: does not qualify for symsim");
            }
          } else if(SymSimDebug) {
            klee_warning("[symsim] do not execute SymSim (prefix mismatch?)");
          }
      } else if(SymSimDebug && bi->getFunction() && std::find(SymSimIgnore.begin(), SymSimIgnore.end(), bi->getFunction()->getName()) != SymSimIgnore.end()) {
        klee_warning("[symsim] ignore symbolic condition for SymSim, fork instead");
      }
      if(doFork) {
        // only counts current number of symsim-style loop explorations
        // => reset after fork.
        // always resetting is faster than looking up loop stuff again
        theStatisticManager->setIndexedValue(stats::symSimLoopCount, ki->info->id, 0);
  ...
  case Instruction::Invoke: {
    if(SymSim && state.symsim) {
      const auto firstBrCurFunc = std::find_if(state.symBranches.begin(),state.symBranches.end(),
                                         [i](const BranchInst *m) -> bool {
                                           return m->getFunction() == i->getFunction();
                                         });
      if(firstBrCurFunc != state.symBranches.end()) {
        abortSim(state, *firstBrCurFunc,
                 std::distance(state.symBranches.begin(), firstBrCurFunc));
      }
    }
  }
  case Instruction::Call: {
    ...
    if(f &&
        (f->getName()=="_ZNK7sc_core8sc_event11add_dynamicEPNS_17sc_thread_processE"
         || f->getName()=="_ZNK7sc_core8sc_event11add_dynamicEPNS_17sc_method_processE")) {
      if(arguments.size() != 2) {
        klee_warning("[symsim-sysc] event.add_dynamic did not have exactly 2 arguments. Can't log properly, expect SymSim issues.");
      } else {
        uint64_t event = getConcreteAddress(state, arguments[0], true);
        state.threadEventsD[state.curThreadID].push_back(event);
      }
    } else if(f &&
               (f->getName()=="_ZNK7sc_core8sc_event10add_staticEPNS_17sc_thread_processE"
                || f->getName()=="_ZNK7sc_core8sc_event10add_staticEPNS_17sc_method_processE")) {
      if (arguments.size() != 2) {
        klee_warning("[symsim-sysc] event.add_static did not have exactly 2 arguments. Can't log properly, expect SymSim issues.");
      } else {
        uint64_t event = getConcreteAddress(state, arguments[0], true);
        state.threadEventsS[state.curThreadID].push_back(event);
      }
    } else if(f &&
               (f->getName() == "_ZN7sc_core12sc_port_base4bindERNS_12sc_interfaceE")) {
      if(arguments.size() != 2) {
        klee_warning("[symsim-sysc] port.bind did not have exactly 2 arguments. Can't log properly, expect SymSim issues.");
      } else {
        uint64_t signal = getConcreteAddress(state, arguments[1], true);
        uint64_t port = getConcreteAddress(state, arguments[0], true);
        state.portSignal[port].push_back(signal);
        state.portSignal[signal].push_back(port);
      }
    }
...
bool Executor::checkSymSimPrefixes(std::string file) {
  bool execute = false; // execute SymSim?
  for(auto s : SymSimPrefixes) {
    if(file.compare(0, s.size(), s) == 0) {
      execute = true;
      break;
    }
  }
  if(SymSimDebug) {
    std::string m = (execute ? std::string("") : std::string("do not ")) + "execute SymSim for " + file;
    klee_warning(m.c_str());
  }
  return execute;
}
...
void Executor::checkThreadNew(ExecutionState &state) {
  if(state.newThread) { // need to jump over the ptc-exec hack, set mainPC/Stack now
    if(ThreadDebug) klee_warning("[threading] check thread new");
...
void Executor::checkThreads(ExecutionState &state) {
  if (state.newThreadID) {
    if(ThreadDebug) klee_warning("[threading] check threads: new thread, register condition");
    ...
    if(ThreadDebug) klee_warning("[threading] check threads: enter");
    if(state.symsim) {
      abortSim(state, state.symBranches.front(), 0);
    }
    if(state.curThreadID) { // not currently in main
      if(ThreadDebug) klee_warning("[threading] enter from thread");
      ...
      if(state.threadPCs.find(state.enterThreadID)!=state.threadPCs.end()) {
        if(ThreadDebug) klee_warning("[threading] enter other thread");
        ...
        state.threadEventsD[state.enterThreadID].clear();
      } else { // enter main
        if(ThreadDebug) klee_warning("[threading] enter main");
        state.pc = state.mainPC;
        state.stack = state.mainStack;
        state.curThreadID = 0;
      }
    } else { // from main
      if(ThreadDebug) klee_warning("[threading] enter from main");
      state.mainPC = state.pc;
      state.mainStack = state.stack;
      state.pc = state.threadPCs[state.enterThreadID];
      state.stack = state.threadStacks[state.enterThreadID];
...
void Executor::trySimBranch(ExecutionState &state, bool init,
                            BasicBlock *t, BasicBlock *f) {
  // execute true next if initialisation of this symbranch and...
  // (a) both true and false have actual paths OR
  // (b) only true has an actual path
  // else target false-branch. might not actually execute (see kill)
  bool nextbranch = init ? true : !state.simt.back();
  unsigned entries = init ? 0 : nextbranch ? 2 : 1;

  bool branchFailed = false;
  bool done = false; // done with one branch of br point
  // done with branch point if both branches were tried
  bool kill = entries == 2;
  bool killstate = false; // no possible paths on this executionstate
  while(!(kill || done || killstate)) {
    if(SymSimDebug) {
      if (nextbranch) klee_warning("[symsim] jump 2 true");
      else klee_warning("[symsim] jump 2 false");
    }
    state.constraints = state.branchBackup.back(); // reset
    if (addConstraint(state, (nextbranch ? state.symCond.back() :
                                         Expr::createIsZero(state.symCond.back())
                              ))) {
      // successfully jumped, all good
      if(!init) state.simt.pop_back();
      state.simt.push_back(nextbranch);
      transferToBasicBlock(nextbranch ? t : f, state.symBranches.back()->getParent(), state);
      done = true;
      ...
      if(SymSimDebug) klee_warning("[symsim] cant jump, go to other or terminate state?");
      // kill state if both branches fail [not possible?]
      if(branchFailed) killstate = true;
      // kill path if other one was already successful (continue there)
      else if(entries==1) kill = true;
      else branchFailed = true; // otherwise just try other one...
    }
    nextbranch = !nextbranch;
  }
  if(killstate) {
    if(SymSimDebug) klee_warning("[symsim] kill state");
    terminateStateOnExit(state);
    return;
  } else if(kill) {
    if(SymSimDebug) klee_warning("[symsim] kill");
    state.symBranches.pop_back();
    state.symsim = !state.symBranches.empty();
    if(SymSimDebug && state.symsim) klee_warning("[symsim] continue symsim");
    if(!init) state.simt.pop_back();
    state.symCond.pop_back();
    state.constraints = state.branchBackup.back();
    state.branchBackup.pop_back();
  }

  if(SymSimDebug) {
    std::string msg("[symsim] symsim branches: ");
    llvm::raw_string_ostream os(msg);
    for (auto br : state.symBranches) {
      const InstructionInfo &inf = kmodule->infos->getInfo(*br);
      std::string infostr =
          "id: " + std::to_string(inf.id) +
          ", line: " + std::to_string(inf.line) +
          ", assmbly line: " + std::to_string(inf.assemblyLine) +
          ", source file name: " + inf.file + "\n";
      os << infostr;
    }
    klee_warning("%s", os.str().c_str());
  }
...
bool Executor::abortSim(ExecutionState &state, BranchInst *simBase, unsigned indexBase) {
  if(SymSimDebug) {
    std::string msgLocal(
        "[symsim-abort] start symsim-abort at ");
    llvm::raw_string_ostream outputLocal(msgLocal);
    std::string infoPC = "state: " + std::to_string(state.getID()) +
        "[prevPC] id: " +
        std::to_string(state.prevPC->info->id) + ", line: " +
        std::to_string(state.prevPC->info->line) +
        ", assmbly line: " +
    ...
    std::string msg("[symsim-abort] symsim branches: ");
    llvm::raw_string_ostream os(msg);
    for (auto br : state.symBranches) {
      const InstructionInfo &inf = kmodule->infos->getInfo(*br);
      std::string infostr =
  ...
  BranchInst *bi = simBase;
  state.constraints = state.branchBackup[indexBase];

  bool curSimPossible = true;
  for(unsigned i=indexBase;i<state.symCond.size();++i) {
    bi = state.symBranches[i];
    Executor::StatePair branches =
        fork(state, state.symCond[i], false, BranchType::Conditional, state.simt[i]);

    if (branches.first) {
      if(SymSimDebug) klee_warning("[symsim-abort] true branch is possible");

      if (!state.simt[i]) {
        if(SymSimDebug) klee_warning("[symsim-abort] was not in symsim-true on abort");
        while (branches.first->stack.back().kf->function != bi->getFunction()) {
          branches.first->popFrame();
        }
        KFunction *fun = kmodule->functionMap[bi->getFunction()];
        BasicBlock *t = bi->getSuccessor(0);
        BasicBlock *f = bi->getSuccessor(1);
        BasicBlock *end = fun->postDom.findNearestCommonDominator(t, f);
        unsigned entry = fun->basicBlockEntry[end];
        branches.first->pc = &fun->instructions[entry];
      }
    }
    if (branches.second) {
      if(SymSimDebug) klee_warning("[symsim-abort] false branch is possible");
      if (state.simt[i]) {
        if(SymSimDebug) klee_warning("[symsim-abort] was not in symsim-false on abort");
        while (branches.second->stack.back().kf->function != bi->getFunction()) {
          branches.second->popFrame();
        }
        transferToBasicBlock(bi->getSuccessor(1), bi->getParent(),
                             *branches.second);
      }
    }
    if ((!branches.first && state.simt[i]) || (!branches.second && !state.simt[i])) {
      if(SymSimDebug) klee_warning("[symsim-abort] current symsim path not possible");
      assert((state.simt[i] ? (branches.second!=nullptr) : (branches.first!=nullptr)) && "no branch possible?");
      curSimPossible = false;
    }
  }
  for(auto s : addedStates) {
    s->branchBackup.erase(s->branchBackup.begin()+indexBase,s->branchBackup.end());
    s->symBranches.erase(s->symBranches.begin()+indexBase,s->symBranches.end());
    s->simt.erase(s->simt.begin()+indexBase,s->simt.end());
    s->symsim = !s->simt.empty();
    s->symCond.erase(s->symCond.begin()+indexBase,s->symCond.end());
  }
  state.branchBackup.erase(state.branchBackup.begin()+indexBase,state.branchBackup.end());
  state.symBranches.erase(state.symBranches.begin()+indexBase,state.symBranches.end());
  state.simt.erase(state.simt.begin()+indexBase,state.simt.end());
  state.symsim = !state.simt.empty();
  state.symCond.erase(state.symCond.begin()+indexBase,state.symCond.end());

  return curSimPossible;
}

bool Executor::checkBB(ExecutionState &state, KInstruction *ki) {
  if(!SymSim) return false;
  if(!state.symsim) return false;
  Instruction *inst = ki->inst;
  bool first = inst == inst->getParent()->getFirstNonPHI();
  if(!first) return false;
  KFunction *fun = kmodule->functionMap[inst->getFunction()];

  Function *basefun = state.symBranches.back()->getFunction();
  if (fun->function != basefun) return false; // if,else,end are obv in same function

  BasicBlock *t = state.symBranches.back()->getSuccessor(0);
  BasicBlock *f = state.symBranches.back()->getSuccessor(1);
  BasicBlock *end = fun->postDom.findNearestCommonDominator(t,f);
  if(inst->getParent()!=end) return false;

//  klee_warning("an end is in sight");
  trySimBranch(state, false, t, f);
  return true;
}
...
ref<Expr> Executor::getConditionalExpression(ExecutionState &state, ref<Expr> value, ref<Expr> offset, Expr::Width type, const MemoryObject *mo, const ObjectState *os) {
  ref<Expr> simcond;
  unsigned count = 0;
  for(auto cond : state.symCond) {
    if(count==0) simcond = state.simt[count] ? cond :
                                  EqExpr::createIsZero(cond);
    else simcond = AndExpr::create(simcond,
                                state.simt[count] ? cond :
                                                  EqExpr::createIsZero(cond));
    ++count;
  }
  return SelectExpr::create(simcond,value,(ConstArrayThreshold >= 0 && mo->size > ConstArrayThreshold) ? read(state, os, offset, type) : os->read(offset,type));
}
      ...
      if (inBounds) {
        ...
        if (isWrite) {
          if (os->readOnly) {
            ...
            if(SymSim && state.symsim) {
              if(!isa<ConstantExpr>(value) || !isa<ConstantExpr>(offset)) {
                value = getConditionalExpression(state, value, offset, type, mo, os);
              } else if(isa<ConstantExpr>(offset)
                         && isa<ConstantExpr>(value) && !mo->isGlobal) {
                if(const Instruction *allocsite =
                        dyn_cast<Instruction>(mo->allocSite)) {
                  if(state.stack.back().kf->function == allocsite->getFunction()) {
                    if(state.symBranches.back()->getFunction() == allocsite->getFunction()) {
                      // find first symsim branch in current function
                      auto firstBrCurFunc = std::find_if(state.symBranches.begin(),state.symBranches.end(),
                                                         [allocsite](const BranchInst *m) -> bool {
                                                           return m->getFunction() == allocsite->getFunction();
                                                         });
                      if(TurnSymbolicLocal && checkSymSimPrefixes(state.prevPC->info->file)) {
                        if(SymSimDebug) {
                          std::string msgLocal(
                              "[symsim] turn symbolic local invoked at ");
                          llvm::raw_string_ostream outputLocal(msgLocal);
                          std::string infoPC = "state: " + std::to_string(state.getID()) +
                              ", [prevPC] id: " +
                              std::to_string(state.prevPC->info->id) + ", line: " +
                        ...
                        value = getConditionalExpression(state, value, offset, type, mo, os);
                      } else if(!abortSim(state, *firstBrCurFunc,
                                           std::distance(state.symBranches.begin(),firstBrCurFunc))) return;
                    }
                  } else {
                    if(TurnSymbolicGlobal && checkSymSimPrefixes(state.prevPC->info->file)) {
                      if(SymSimDebug) {
                        std::string msgGlobal(
                            "[symsim] turn symbolic global invoked at ");
                        llvm::raw_string_ostream outputGlobal(msgGlobal);
                        std::string infoPC = "state: " + std::to_string(state.getID()) +
                            ", [prevPC] id: " +
                            std::to_string(state.prevPC->info->id) +
                            ", line: " + std::to_string(state.prevPC->info->line) +
                            ", assembly line: " +
                            std::to_string(state.prevPC->info->assemblyLine) +
                            ", source file name: " + state.prevPC->info->file;
                        outputGlobal << infoPC;
                        klee_warning("%s", outputGlobal.str().c_str());
                      }
                      value = getConditionalExpression(state, value, offset, type, mo, os);
                    } else if(!abortSim(state, state.symBranches.front(), 0)) return;
                  }
                } else if(SymSimDebug) {
                  klee_warning("[symsim] alloc site is not an instruction?");
                }
              } else if(mo->isGlobal) {
                if(SymSimDebug) klee_warning("[symsim] unhandled concrete write to *global* memory object, abort");
                abortSim(state, state.symBranches.front(), 0);
              }
            }
