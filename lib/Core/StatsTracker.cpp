...
cl::list<std::string> CoveragePrefixes(
    "coverage-prefix",
    cl::desc("Only count coverage in files that match prefix(es)"),
    cl::value_desc("relative directory/file name"),
    cl::cat(StatsCat));

cl::opt<bool> CoverageDebug("coverage-debug", cl::init(false),
                            cl::desc("Debug output for prefix-coverage"),
                            cl::cat(StatsCat));
    ...
    for (unsigned i=0; i<kf->numInstructions; ++i) {
      ...
      if (OutputIStats) {
        ...
        if (kf->trackCoverage && instructionIsCoverable(ki->inst)) {
          bool ignore = !CoveragePrefixes.empty();
          for(auto s : CoveragePrefixes) {
            if(ki->info->file.compare(0, s.size(), s) == 0) {
              ignore = false;
              break;
            }
          }
          if(!ignore) {
            if(CoverageDebug && !CoveragePrefixes.empty())
              klee_warning("[uncovered] matches prefix: %s", ki->info->file.c_str());
      ...
      if (kf->trackCoverage) {
        if (BranchInst *bi = dyn_cast<BranchInst>(ki->inst))
          if (!bi->isUnconditional()) {
            ...
            if(!ignore) {
              if(CoverageDebug && !CoveragePrefixes.empty())
                klee_warning("[br-new] matches prefix: %s", ki->info->file.c_str());
              numBranches++;
            }
          }
      }
  ...
  if (OutputIStats) {
    ...
    if (sf.kf->trackCoverage && instructionIsCoverable(inst)) {
      if (!theStatisticManager->getIndexedValue(stats::coveredInstructions, ii.id)) {
        ...
        bool ignore = !CoveragePrefixes.empty();
         for(auto s : CoveragePrefixes) {
           if(ii.file.compare(0,s.size(),s)==0) {
             ignore = false;
             break;
           }
        }
        if(!ignore) {
          if(CoverageDebug && !CoveragePrefixes.empty())
            klee_warning("[covered] matches prefix: %s", ii.file.c_str());
...
void StatsTracker::markBranchVisited(unsigned id,
  ...
  if (OutputIStats) {
    ...
    if (visitedTrue && !hasTrue) {
      bool ignore = !CoveragePrefixes.empty();
      for(auto s : CoveragePrefixes) {
        if(visitedTrue->pc->info->file.compare(0, s.size(), s) == 0) {
          ignore = false;
          break;
        }
      }
      if(!ignore) {
        if(CoverageDebug && !CoveragePrefixes.empty())
          klee_warning("[br-true] matches prefix: %s", visitedTrue->pc->info->file.c_str());
    ...
    if (visitedFalse && !hasFalse) {
      bool ignore = !CoveragePrefixes.empty();
      for(auto s : CoveragePrefixes) {
        if(visitedFalse->pc->info->file.compare(0, s.size(), s) == 0) {
          ignore = false;
          break;
        }
      }
      if(!ignore) {
        if(CoverageDebug && !CoveragePrefixes.empty())
          klee_warning("[br-false] matches prefix: %s", visitedFalse->pc->info->file.c_str());
...
#endif
  sqlite3_bind_int64(insertStmt, arg++, stats::symSimLoopCount);
