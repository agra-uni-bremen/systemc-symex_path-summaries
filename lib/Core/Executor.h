...
private:
  ...
  bool checkSymSimPrefixes(std::string file);

  // attempt to set state attribute (simt, pc, ...) to execute nextbranch
  // entries=0: just arrived at new symsim point
  // entries=1: already done with symsim for true, false next
  // entries=2: already done with both, kill
  void trySimBranch(ExecutionState &state, bool init, llvm::BasicBlock *t,
                    llvm::BasicBlock *f);
  // check if end of true/false symsim branch is reached
  bool checkBB(ExecutionState &state, KInstruction *ki);
  // abort symsim, starting at simBase (simbase will be first condition handed to fork)
  bool abortSim(ExecutionState &state, llvm::BranchInst *simBase, unsigned indexBase);

  ref<Expr> getConditionalExpression(ExecutionState &state, ref<Expr> value,
                                     ref<Expr> offset, Expr::Width type,
  ...
  StatePair fork(ExecutionState &current, ref<Expr> condition, bool isInternal,
                 BranchType reason, bool former=true);
...
public:
  ...
  /// input: ref<Expr> representing concrete memory address
  ///         and bool to decide whether zero offset is allowed (true) or
  ///         should lead to return 0 (false)
  /// return: concrete value representing memory address
  uint64_t getConcreteAddress(klee::ExecutionState &state, ref<Expr> expr, bool zeroOffset);
};
