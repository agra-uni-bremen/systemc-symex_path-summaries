...
void KModule::infiniteLoopDummies() {
  klee::infiniteLoopDummies(module.get());
}
...
KFunction::KFunction(llvm::Function *_function,
  ...
  // no (post)dom for empty function (e.g. special functions)
  if(numInstructions==0) {
    return;
  }

  postDom = PostDominatorTree(*function);
  loops = LoopInfo(DominatorTree(*function));
}
